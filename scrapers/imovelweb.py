...
logging.basicConfig(
    filename=os.path.join(log_dir, f"scraper_imovelweb_{datetime.now().strftime('%Y%m%d')}.log"),
    level=logging.INFO,
...
def executar_scraping_imovelweb(estrutura_aps):
    if not API_KEY:
        logging.error("SCRAPINGBEE_API_KEY não configurada no ambiente.")
        return

    data_atual = datetime.now().strftime("%Y-%m-%d")
    output_dir = os.path.join("data", "raw", "imovelweb", data_atual)
